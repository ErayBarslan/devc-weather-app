  ...
  const fetchData = async () => {
    if (!city) {
      try {
        ...
        const res2 = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${data.latitude}&lon=${data.longitude}&appid=${process.env.API_KEY}`)
        const data2 = await res2.json()
        setCurrentData(data2)
        const res3 = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${data.latitude}&lon=${data.longitude}&appid=${process.env.API_KEY}`)
      ...
      try {
        setLoading(true)
        const res = await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=5&appid=${process.env.API_KEY}`)
        const data = await res.json()
        const res2 = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${data[0].lat}&lon=${data[0].lon}&appid=${process.env.API_KEY}`)
        const data2 = await res2.json()
        setCurrentData(data2)
        const res3 = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${data[0].lat}&lon=${data[0].lon}&appid=${process.env.API_KEY}`)
        const data3 = await res3.json()
        setForecastData(data3)
        setCity("")
        setLoading(false)
